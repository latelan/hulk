...
/**
 * Description of FrameDI
 * Frame的DI容器
 * @author zhangjiulong
 */
class FrameDI extends FrameObject {
...
//            throw new ExceptionFrame('没有在容器中找到' . $id . '对应的对象');
        } else {
            return null;
        }
    }
    ...
    public function set($id, $definition) {
        ...
        if (is_object($definition) || is_callable($definition, true)) {
            ...
            if (isset($definition['class'])) {
                ...
                throw new ExceptionFrame('the configuration for the "' . $id . '" object must contain "class" element');
//                throw new ExceptionFrame('component ' . $id . ' 的配置数组中必须包含“class”元素');
            }
        } else {
            throw new ExceptionFrame('the configuration type for the "' . $id . '" object error: ' .  gettype($definition));
//            throw new ExceptionFrame('component ' . $id . ' 的配置类型(' . gettype($definition) . ')错误');
        }
    }
