...
/**
 * Description of FrameMemcache
 * Memcache缓存类
 * @author zhangjiulong
 */
class FrameMemcache extends BaseCache {
    ...
    /**
     * memcache的服务器配置，参数见 FrameMemecacheServer
     * @var array 
     */
    private $_servers = [];
    ...
    {
        if (empty($servers)) {
            $servers = [new FrameMemcacheServer(['host' => '127.0.0.1'])];
        } else {
            foreach ($servers as $server) {
                if (!$server->host) {
                    throw new ExceptionFrame('the host of the server is required');
                }
            }
        }
        foreach ($servers as $server) {
    ...
    public function getMemcache()
    {
        if ($this->_cache === null) {
            ...
            if (!extension_loaded($extension)) {
                throw new ExceptionFrame('Memcache required php extension ' . $extension);
            }
            if ($this->use_memcached) {
    ...
    public function setServers($servers)
    {
        foreach ($servers as $config) {
            $this->_servers[] = new FrameMemcacheServer($config);
        }
    }
...
class FrameMemcacheServer extends FrameObject {
