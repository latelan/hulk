...
class FrameRedis extends FrameObject {
    ...
    /**
     * @var array redis命令列表 http://redis.io/commands
     */
    ...
    /**
     * 打开redis连接
     * @throws ExceptionFrame 连接失败抛出异常
     */
    public function open()
    {
        ...
        //@todo log 
        FrameLog::info('Opening redis DB connection: ' . $connection, 'redis.connect');
        ...
        if ($this->_socket) {
            ...
            //@todo log
            FrameLog::error("Failed to open redis DB connection ($connection): $errorNumber - $errorDescription", 'redis');
            throw new ExceptionFrame('Failed to open DB connection.', (int) $errorNumber);
        }
    }
    ...
    public function close()
    {
        if ($this->_socket !== null) {
            $connection    = ($this->unix_socket ? : $this->hostname . ':' . $this->port) . ', database=' . $this->database;
            //@todo log
            FrameLog::info('Closing DB connection: ' . $connection, 'redis.connect');
    ...
    public function __call($name, $params)
    {
        ...
        if (in_array($redisCommand, $this->redisCommands)) {
            ...
            throw new ExceptionFrame('call to unknow redis command ' . $name);
        }
    }
     ...
     * @trows ExceptionFrame 当命令返回 [error reply]的时候(http://redis.io/topics/protocol#error-reply).
     */
    public function executeCommand($name, $params = [])
    {
        ...
        //@todo log
        FrameLog::info(['command' => $name, 'params' => $params], 'redis.execute');
     ...
     * @return mixed
     * @throws ExceptionFrame on error
     */
    private function parseResponse($command)
    {
        if (($line = fgets($this->_socket)) === false) {
            throw new ExceptionFrame("Failed to read from socket.\nRedis command was: " . $command);
        }
        //@todo log
        FrameLog::debug($command . "\n" . $line, 'redis.response');
        $type = $line[0];
        ...
        switch ($type) {
            ...
            case '-': // Error reply
                throw new ExceptionFrame("Redis error: " . $line . "\nRedis command was: " . $command);
            case ':': // Integer reply
            ...
            case '*': // Multi-bulk replies
                ...
                return $data;
            default:
                throw new ExceptionFrame('Received illegal data from redis: ' . $line . "\nRedis command was: " . $command);
        }
    }
