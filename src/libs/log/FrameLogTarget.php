...
/**
 * Description of FrameLogTarget
 *
 * @author zhangjiulong
 */
abstract class FrameLogTarget extends FrameObject {
    ...
    public function setLevels($levels) {
        static $levelMap = [
            'error' => FrameLog::LEVEL_ERROR,
            'info' => FrameLog::LEVEL_INFO,
            'debug' => FrameLog::LEVEL_DEBUG,
        ];
        if (is_array($levels)) {
            $this->_levels = 0;
            foreach ($levels as $level) {
                if (isset($levelMap[$level])) {
                    ...
                    throw new ExceptionFrame('unknow log level: ' . $level);
                }
            }
        } else {
            $this->_levels = $levels;
